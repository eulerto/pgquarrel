...
$PGPATH1/psql -p $PGPORT1 -X -f test-server1.sql postgres
$PGPATH2/psql -p $PGPORT2 -X -f test-server2.sql postgres
# run pgquarrel
$HOME/pgquarrel/bin/pgquarrel -c test.ini
# apply differences
$PGPATH1/psql -p $PGPORT1 -X -f /tmp/test.sql quarrel1
# test again
$HOME/pgquarrel/bin/pgquarrel -c test2.ini
# comparing dumps
