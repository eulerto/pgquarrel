...
getTextSearchConfigs(PGconn *c, int *n)
{
	...
	*n = PQntuples(res);
	...
	for (i = 0; i < *n; i++)
	{
		...
		logDebug("text search configuration \"%s\".\"%s\"", d[i].obj.schemaname, d[i].obj.objectname);
	}
...
getTextSearchDicts(PGconn *c, int *n)
{
	...
	do
	{
		...
		r = snprintf(query, nquery, "SELECT d.oid, n.nspname, d.dictname, d.dictinitoption, quote_ident(o.nspname) || '.' || t.tmplname AS tmplname, obj_description(d.oid, 'pg_ts_dict') AS description, pg_get_userbyid(d.dictowner) AS dictowner FROM pg_ts_dict d INNER JOIN pg_namespace n ON (d.dictnamespace = n.oid) INNER JOIN pg_ts_template t ON (d.dicttemplate = t.oid) INNER JOIN pg_namespace o ON (t.tmplnamespace = o.oid) WHERE d.oid >= %u ORDER BY n.nspname, d.dictname", PGQ_FIRST_USER_OID);
	...
	*n = PQntuples(res);
	...
	for (i = 0; i < *n; i++)
	{
		...
		logDebug("text search dictionary \"%s\".\"%s\"", d[i].obj.schemaname, d[i].obj.objectname);
	}
...
getTextSearchParsers(PGconn *c, int *n)
{
	...
	*n = PQntuples(res);
	...
	for (i = 0; i < *n; i++)
	{
		...
		logDebug("text search parser \"%s\".\"%s\"", p[i].obj.schemaname, p[i].obj.objectname);
	}
...
getTextSearchTemplates(PGconn *c, int *n)
{
	...
	do
	{
		...
		r = snprintf(query, nquery, "SELECT t.oid, n.nspname, t.tmplname, quote_ident(o.nspname) || '.' || quote_ident(a.proname) AS tmpllexize, quote_ident(p.nspname) || '.' || quote_ident(b.proname) AS tmplinit, obj_description(t.oid, 'pg_ts_template') AS description FROM pg_ts_template t INNER JOIN pg_namespace n ON (t.tmplnamespace = n.oid) INNER JOIN (pg_proc a INNER JOIN pg_namespace o ON (a.pronamespace = o.oid)) ON (t.tmpllexize = a.oid) LEFT JOIN (pg_proc b INNER JOIN pg_namespace p ON (b.pronamespace = p.oid)) ON (t.tmplinit = b.oid) WHERE t.oid >= %u ORDER BY n.nspname, t.tmplname", PGQ_FIRST_USER_OID);
	...
	*n = PQntuples(res);
	...
	for (i = 0; i < *n; i++)
	{
		...
		logDebug("text search template \"%s\".\"%s\"", t[i].obj.schemaname, t[i].obj.objectname);
	}
...
void
dumpAlterTextSearchConfig(FILE *output, PQLTextSearchConfig *a, PQLTextSearchConfig *b)
{
