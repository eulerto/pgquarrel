 ...
 * ALTER TABLE ... ADD COLUMN ... UNIQUE [ NULLS [ NOT ] DISTINCT ]
...
getTableAttributes(PGconn *c, PQLTable *t)
{
	...
	char *and_exclude_table_column_str = "";
	char *left_exclude_table_column_str = "";
	if (strlen(exclude_table_column_str) > 0)
	{
		if (PQserverVersion(c) >= 90400)
		{
			left_exclude_table_column_str = psprintf(" LEFT JOIN (SELECT * FROM json_to_recordset('%s') as x(t text, c text)) f ON f.t = a.attrelid::regclass::text AND f.c = a.attname", exclude_table_column_str);
			and_exclude_table_column_str = " AND (f.t IS NULL OR a.attrelid::regclass::text <> f.t OR a.attname <> f.c)";
		}
	}
